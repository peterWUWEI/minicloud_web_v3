...
onMounted(async () => {
  others.value = await DataService.findById(route.params.id);
  thumbnailUrl.value = others.value.image_url;
  currData.value = others.value.data;
});
...
<template>
  ...
  <div class="card">
    <form @submit.prevent="updateOthers">
      <Tabs :value="tabValue">
        <TabList>
