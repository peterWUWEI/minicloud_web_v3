...
const selectedOther = ref({ name: '可持续发展', path: 'sustainability' });
const others = ref([
  { name: '可持续发展', path: 'sustainability' },
  { name: '投资者信息', path: 'investorInfo' }
]);
...
const createNewOthers = async () => {
  try {
    await DataService.put({
      category: `${selectedOther.value.path}`,
      id: newId.value,
      data: newData.value,
...
<template>
  ...
  <div class="card">
    <form @submit.prevent="createNewOthers">
      <Tabs :value="tabValue">
        <TabList>
